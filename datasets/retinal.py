"""
Retinal dataset loader for Tip-Adapter
Loads from HuggingFace datasets:
- Dataset: andyqmongo/IVL_OOD_retinal
- Train split: 1_shot
- Test split: test
"""
import os
...
template = ['a retinal image showing {}.', 'a fundus photo of {}.', 'an ophthalmology image of {}.']
...
class Retinal(DatasetBase):
    """Retinal classification dataset from HuggingFace."""
    ...
    def __init__(self, root, num_shots):
        ...
        train, val, test = self.load_retinal_datasets()
    ...
    def load_retinal_datasets(self):
        """Load Retinal datasets from HuggingFace."""
        print("Loading Retinal datasets from HuggingFace...")
        
        train_dataset = _load_hf_dataset("andyqmongo/IVL_OOD_retinal", "8_shot")
        test_dataset = _load_hf_dataset("andyqmongo/IVL_OOD_retinal", "test")
    ...
    def _get_classname(self, item):
        ...
        elif 'diagnosis' in item and item['diagnosis'] is not None:
            return str(item['diagnosis']).lower().replace('_', ' ')
        elif 'condition' in item and item['condition'] is not None:
            return str(item['condition']).lower().replace('_', ' ')
        else:
