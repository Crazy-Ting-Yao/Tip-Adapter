"""
WM811k Wafer Map dataset loader for Tip-Adapter
...
template = ['a wafer map showing {} defect pattern.', 'a {} pattern on wafer map.', 'wafer defect pattern: {}.']
...
class WM811k(DatasetBase):
    """WM811k Wafer Map defect classification dataset from HuggingFace."""
    ...
    def __init__(self, root, num_shots):
        ...
        train, val, test = self.load_wm811k_datasets()
    ...
    def load_wm811k_datasets(self):
        """Load WM811k datasets from HuggingFace."""
        print("Loading WM811k datasets from HuggingFace...")
        
        train_dataset = _load_hf_dataset("andyqmongo/IVL_OOD_WM811k", "8_shot")
        test_dataset = _load_hf_dataset("andyqmongo/IVL_OOD_WM811k", "test")
    ...
    def _get_classname(self, item):
        ...
        elif 'defect' in item and item['defect'] is not None:
            return str(item['defect']).lower().replace('_', ' ')
        elif 'pattern' in item and item['pattern'] is not None:
            return str(item['pattern']).lower().replace('_', ' ')
